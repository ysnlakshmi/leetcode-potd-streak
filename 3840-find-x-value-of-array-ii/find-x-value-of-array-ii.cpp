...
private:
    ...
    void update(int treeIndex, int lo, int hi, int i, int val) {
        if (lo == hi) {
            for (int j = 0; j < k; ++j) tree[treeIndex].remain[j] = 0;
            tree[treeIndex].remain[val] = 1;
            tree[treeIndex].prod = val;
            return;
        }
        const int mid = (lo + hi) / 2;
        if (i <= mid) update(2 * treeIndex + 1, lo, mid, i, val);
        else update(2 * treeIndex + 2, mid + 1, hi, i, val);
    ...
    Node query(int treeIndex, int lo, int hi, int i, int j) const {
        ...
        if (j < lo || hi < i)
            return Node(); // Returns identity element (prod = 1, remain counts = 0)
        const int mid = (lo + hi) / 2;
...
public:
    vector<int> resultArray(vector<int>& nums, int k, vector<vector<int>>& queries) {
        ...
        for (vector<int>& query : queries) query[1] %= k;
